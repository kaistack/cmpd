...
for (k = 1; dstate[k] != 0; k++) { m = dstate[k];
if (m > 10) {
q = m / 10;
r = m % 10;
} else {
...
for (j = 0; j < 2; j++) { dfa[k][j] = nfa[r][j]; flag = 0;
for (int i = 1; i <= n; i++) { if (dfa[k][j] == dstate[i]) {
flag = 1; break;
}
}
if (!flag && dfa[k][j] != 0) { int nextState = dfa[k][j]; int duplicate = 0;
for (int l = 1; l <= n; l++) {
if (dstate[l] == nextState) { duplicate = 1;
break;
}
}
if (!duplicate) { dstate[++n] = nextState;
}
}
}
}
